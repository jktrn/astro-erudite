...
- The breadcrumb component required careful thought to handle three distinct cases:
    ...
    ```astro title="src/pages/blog/[...id].astro" startLineNumber={70} {"For subposts: Blog > Parent > Subpost":6-16} {"For parent posts: Blog > Parent > Index":20-30} {"For regular posts: Blog > Post":33-38}
    <Breadcrumbs
    items={[
    ...
    Showing parent posts (that have subposts) as "Parent > Index" rather than just "Parent" makes it more clear that the parent post itself is content, not just a container.
